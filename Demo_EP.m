% Demo on Exponential Power Noise
tic; clear;clc;
addpath(genpath(pwd));
...
% PMoEP
CC_PMoEP = 0.12;            % tune between(0,0.3]
...
for kk = 1:data_num    
    ...
    noise = rnormp(m*n-p1,0,0.2,0.2);
   
    X_Noi(Ind(p1+1:end)) = X_Noi(Ind(p1+1:end)) + noise;   % Noisy data
