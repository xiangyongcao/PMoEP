...
for kk = 1:data_num
    ...
    p2 = floor(m*n*0.3); 
    X_Noi(Ind(p1+1:p1+p2)) = X_Noi(Ind(p1+1:p1+p2)) + rnormp(p2,0,0.1,0.5); % Add EP0.5 noise
    p3 = floor(m*n*0.4);
    X_Noi(Ind(p1+p2+1:p1+p2+p3)) = X_Noi(Ind(p1+p2+1:p1+p2+p3)) + randl(1,p3)* 0.3; % Add Laplace noise
