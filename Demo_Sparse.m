...
% PMoEP
Lambda_PMoEP = 0.02;        % tune between(0,0.3]
len_lam_PMoEP = length(Lambda_PMoEP);
...
for kk = 1:data_num
    ...
    p2 = floor(m*n*0.1);
    noise = rand(1,p2)* 40 - 20;
    
    X_Noi(Ind(p1+1:p1+p2)) = X_Noi(Ind(p1+1:p1+p2)) + noise;   % Add sparse noise
    ...
    for i = 1:init_num
        ...
        for l = 1:len_lam_PMoEP
            ...
            param_PMoEP.k = 4;              % the initialized number of mixture components
            param_PMoEP.display = 0;        % the display setting
            param_PMoEP.tol = 1.0e-10;      % the tolerance
            p=[2,2,2,2,2];                  % can be tuned
...
for kk = 1:data_num
    for i = 1:init_num
        ...
        CCC_PMoEP(kk,i) = Lambda_PMoEP(ind);
        PP{kk,i} = P{i,ind,kk};
    end
end
