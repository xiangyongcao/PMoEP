...
%
% Perform one iteration of the damped Newton algorithm. Provide all
...
if nargin==4
	...
	[last_error,fd,HA,HB,HC] = amb_mfwmd_errfunc_sumsqrderr(state.A,state.B,state.M,state.W);
	
	[last_regerr,fd_sup,HA_sup,HB_sup,HC_sup] = amb_mfwmd_errfunc_regularizeAB(state.A,state.B,state.Lr);
	...
	while improvement<-state.tolerance
		...
		state.error = amb_mfwmd_errfunc_sumsqrderr(state.A,state.B,state.M,state.W);
		state.error = state.error + state.L * amb_mfwmd_errfunc_regularizeAB(state.A,state.B,state.Lr);
